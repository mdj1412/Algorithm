...
// type = 0, 1, 2, 3
void helpFunction(int* clock, int currentSwitch) {
    std::size_t N = switchNumber[currentSwitch].size();
    
    for (int i=0;i<N;i++) {
        clock[switchNumber[currentSwitch][i]] += 3;
        
        if (clock[switchNumber[currentSwitch][i]] == 15)
            clock[switchNumber[currentSwitch][i]] = 3;
    }
}
...
bool recursiveFunction(int* clock, int currentSwitch, int& helpCount) {
    ...
    for (int type=0; type<4; type++) {
        ...
        helpFunction(clock, currentSwitch);
    }
